...
const getLargeTitle = (refEl: any) => {
  const tabs =
    refEl.tagName === 'ION-TABS' ? refEl : refEl.querySelector('ion-tabs');
  const query =
...
export const iosTransitionAnimation = (
  ...
  try {
    ...
    const headerEls = enteringEl.querySelectorAll(
      ':scope > app-header > ion-header > *:not(ion-toolbar), :scope > ion-footer > *'
    );
    const enteringToolBarEls = enteringEl.querySelectorAll(
      ':scope > app-header > ion-header > ion-toolbar'
    );
    ...
    enteringToolBarEls.forEach((enteringToolBarEl) => {
      ...
      const inactiveHeader = parentHeader?.classList.contains(
